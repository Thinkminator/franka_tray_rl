...
# torque rate limiter (you said it's fine)
kDeltaTauMax = 1.0
...
# ---------- Parse mode (and optional seed for 'seeded') ----------
if len(sys.argv) < 2:
    print("Usage: python action.py [zero|seeded|random] [optional_seed_for_seeded]")
    sys.exit(1)
...
if mode == "seeded":
    if len(sys.argv) >= 3:
        try:
            ...
            print("Provided seed is not an integer; using default 12345")
            seed = 12345
    else:
        seed = 12345
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and step_count < max_steps:
        ...
        # --- sample action for this control step ---
        if mode == "zero":
            action_norm = np.zeros(n_joints)
        elif mode == "seeded":
            action_norm = seeded_rng.uniform(-1.0, 1.0, size=n_joints)
        else:  # random
            action_norm = np.random.uniform(-1.0, 1.0, size=n_joints)

        # denormalize to joint angular speeds (rad/s)
        vel_cmd = action_norm * max_speed

        # integrate q_des using denormalized velocity * dt
        delta_q = vel_cmd * dt
        q_des += delta_q
        ...
        # occasional print to monitor action and progress
        if step_count % 200 == 0:
            print(f"[{current_time:.2f}s] step {step_count} action_norm(first4) = {np.round(action_norm[:4],4)} "
                  f"vel_cmd(first4) = {np.round(vel_cmd[:4],4)} q_des(first4) = {np.round(q_des[:4],4)} q_act(first4) = {np.round(current_positions[:4],4)}")
...
if len(timestamps) > 0:
    ...
    for i in range(n_joints):
        axes[0].plot(timestamps, target_trajectories[i], '--', alpha=0.7)
        axes[0].plot(timestamps, actual_positions[i], '-', alpha=0.7)
    axes[0].set_ylabel('Joint Position (rad)')
    axes[0].set_title('Target vs Actual (all joints)')
    axes[0].grid(True)

    for i in range(n_joints):
        axes[1].plot(timestamps, control_signals[i])
    axes[1].set_ylabel('Control signal')
    axes[1].grid(True)

    for i in range(n_joints):
        axes[2].plot(timestamps, torques_applied[i])
