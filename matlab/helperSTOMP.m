...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    [sampled_paths, em] = stompSamples(nPaths, Rinv, theta); % returns noise + paths
    ...
    for k = 1:nPaths
        [S, Q] = stompTrajCost(robot_struct, sampled_paths{k}, R, voxel_world);
        Stheta(k, :) = S;
    end
...
v = VideoWriter('panda_tray_Training.avi');
v.FrameRate = 15;
open(v);
...
enableVideo = 1;
if enableVideo == 1
    % Sanitize robot_name for file systems (replace spaces etc.)
    robot_name = "panda_tray"
    safeName = regexprep(robot_name, '[^\w\-]', '_');  % letters/digits/_/-
    fname = sprintf('%s_final_trajectory.avi', safeName);
...
% Visualize trajectory snapshots (tray axes + object box)
try
    opts = struct();
    opts.trayName = "tray_object";   % must match the body name used in STOMP
    opts.nSnapshots = nDiscretize;   % number of snapshot poses to draw
    opts.arrowScale = 0.06;
    opts.boxSize = [0.04 0.04 0.02];  % change to match your object
    drawTrajectorySnapshots(robot, theta, opts);
catch ME
    warning('drawTrajectorySnapshots failed: %s', ME.message);
end